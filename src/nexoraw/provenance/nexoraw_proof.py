...
def proof_config_from_environment() -> NexoRawProofConfig:
    key = _env_first("NEXORAW_PROOF_KEY")
    if not key:
        ...
        return NexoRawProofConfig(
            ...
            signer_name=_env_first("NEXORAW_PROOF_SIGNER_NAME") or "NexoRAW local signer",
            signer_id=_env_first("NEXORAW_PROOF_SIGNER_ID"),
        )
    public = _env_first("NEXORAW_PROOF_PUBLIC_KEY")
    return NexoRawProofConfig(
        private_key_path=Path(key).expanduser(),
        public_key_path=Path(public).expanduser() if public else None,
        key_passphrase=_env_first("NEXORAW_PROOF_KEY_PASSPHRASE"),
        signer_name=_env_first("NEXORAW_PROOF_SIGNER_NAME") or "NexoRAW local signer",
        signer_id=_env_first("NEXORAW_PROOF_SIGNER_ID"),
    )
