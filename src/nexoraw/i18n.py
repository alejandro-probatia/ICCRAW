"""
Sistema de internacionalización (i18n) para NexoRAW.

Idioma nativo: español (es). Sin fichero .qm cargado, la app muestra español.
Las traducciones se almacenan en src/nexoraw/resources/locales/<lang>.qm
...
1. Extraer cadenas nuevas del código fuente:
       pyside6-lupdate src/nexoraw/gui.py [otros .py] -ts locales/en.ts
...
3. Compilar a binario:
       pyside6-lrelease locales/en.ts -qm src/nexoraw/resources/locales/en.qm
...
def _qm_path(lang: str) -> Path | None:
    ...
    try:
        locale_dir = resources.files("nexoraw.resources").joinpath("locales")
