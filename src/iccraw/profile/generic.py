...
def ensure_generic_output_profile(output_space: str | None, *, directory: Path | None = None) -> Path:
    ...
    if source is None:
        raise RuntimeError(
            ...
            "Instala ArgyllCMS con su directorio ref, configura NEXORAW_STANDARD_ICC_DIR "
            "o selecciona un perfil ICC estandar del sistema."
        )

    base = directory or (Path.home() / ".cache" / "nexoraw" / "standard-profiles")
...
def _standard_profile_search_dirs() -> list[Path]:
    dirs: list[Path] = []
    configured = os.environ.get("NEXORAW_STANDARD_ICC_DIR", "").strip() or os.environ.get(
        "ICCRAW_STANDARD_ICC_DIR", ""
    ).strip()
    if configured:
...
def _argyll_reference_dirs() -> list[Path]:
    dirs: list[Path] = []
    configured = os.environ.get("NEXORAW_ARGYLL_REF_DIR", "").strip() or os.environ.get(
        "ICCRAW_ARGYLL_REF_DIR", ""
    ).strip()
    if configured:
        dirs.append(Path(configured).expanduser())
