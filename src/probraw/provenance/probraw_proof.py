...
PROOF_SCHEMA = "org.probatia.probraw.proof.v1"
LEGACY_PROOF_SCHEMAS = {
    "org.probatia.nexoraw.proof.v1",
    "org.probatia.iccraw.proof.v1",
}
PROOF_ALGORITHM = "Ed25519"
PROOF_CANONICALIZATION = "json-rfc8785-like-sort-keys-v1"


class ProbRawProofError(RuntimeError):
    """Raised when ProbRAW Proof cannot be created or verified."""


@dataclass
class ProbRawProofConfig:
...
def proof_config_from_environment() -> ProbRawProofConfig:
    key = _env_first("PROBRAW_PROOF_KEY", "NEXORAW_PROOF_KEY")
    if not key:
        ...
        return ProbRawProofConfig(
            ...
            signer_name=_env_first("PROBRAW_PROOF_SIGNER_NAME", "NEXORAW_PROOF_SIGNER_NAME") or "ProbRAW local signer",
            signer_id=_env_first("PROBRAW_PROOF_SIGNER_ID", "NEXORAW_PROOF_SIGNER_ID"),
        )
    public = _env_first("PROBRAW_PROOF_PUBLIC_KEY", "NEXORAW_PROOF_PUBLIC_KEY")
    return ProbRawProofConfig(
        private_key_path=Path(key).expanduser(),
        public_key_path=Path(public).expanduser() if public else None,
        key_passphrase=_env_first("PROBRAW_PROOF_KEY_PASSPHRASE", "NEXORAW_PROOF_KEY_PASSPHRASE"),
        signer_name=_env_first("PROBRAW_PROOF_SIGNER_NAME", "NEXORAW_PROOF_SIGNER_NAME") or "ProbRAW local signer",
        signer_id=_env_first("PROBRAW_PROOF_SIGNER_ID", "NEXORAW_PROOF_SIGNER_ID"),
    )
...
def default_proof_sidecar_path(output_tiff: Path) -> Path:
    return Path(output_tiff).with_suffix(Path(output_tiff).suffix + ".probraw.proof.json")


def existing_proof_sidecar_path(output_tiff: Path) -> Path:
    path = default_proof_sidecar_path(output_tiff)
    if path.exists():
        return path
    output_tiff = Path(output_tiff)
    for suffix in (".nexoraw.proof.json", ".iccraw.proof.json"):
        legacy = output_tiff.with_suffix(output_tiff.suffix + suffix)
        if legacy.exists():
            return legacy
    return path
    ...
    *,
    ...
    if private_key.exists() and public_key.exists():
        ...
        if not isinstance(public, Ed25519PublicKey):
            raise ProbRawProofError("La identidad local Proof no usa clave publica Ed25519")
...
def sign_probraw_proof(
    *,
    ...
    if not output_tiff.exists():
        raise FileNotFoundError(f"No existe TIFF final para firmar con ProbRAW Proof: {output_tiff}")
    if not source_raw.exists():
        raise FileNotFoundError(f"No existe RAW fuente para ProbRAW Proof: {source_raw}")
    ...
    payload = build_probraw_proof_payload(
...
def build_probraw_proof_payload(
    *,
    output_tiff: Path,
    source_raw: Path,
    recipe: Recipe,
    ...
    return {
        "schema": PROOF_SCHEMA,
        "schema_version": 1,
        "proof_type": "probraw-ed25519-sidecar-v1",
        ...
        "c2pa": {
            ...
            "is_required_for_probraw_proof": False,
        },
        "forensic_notes": {
...
def verify_probraw_proof(
    ...
    *,
    ...
    if not isinstance(signature_block, dict):
        raise ProbRawProofError("El proof no contiene bloque de firma")
...
def _default_proof_identity_dir() -> Path:
    ...
    return Path.home().expanduser() / ".probraw" / "proof"
...
def _load_private_key(path: Path, passphrase: str | None) -> Ed25519PrivateKey:
    if not Path(path).exists():
        raise FileNotFoundError(f"No existe clave privada ProbRAW Proof: {path}")
    password = passphrase.encode("utf-8") if passphrase else None
    key = serialization.load_pem_private_key(Path(path).read_bytes(), password=password)
    if not isinstance(key, Ed25519PrivateKey):
        raise ProbRawProofError("ProbRAW Proof requiere clave privada Ed25519")
    return key


def _load_public_key(path: Path | None) -> Ed25519PublicKey:
    if path is None:
        raise ProbRawProofError("No se ha indicado clave publica ProbRAW Proof")
    key = serialization.load_pem_public_key(Path(path).read_bytes())
    if not isinstance(key, Ed25519PublicKey):
        raise ProbRawProofError("ProbRAW Proof requiere clave publica Ed25519")
    return key
...
def _public_key_from_payload(payload: dict[str, Any]) -> Ed25519PublicKey:
    ...
    if not public_key_pem:
        raise ProbRawProofError("El proof no contiene clave publica embebida")
    key = serialization.load_pem_public_key(str(public_key_pem).encode("ascii"))
    if not isinstance(key, Ed25519PublicKey):
        raise ProbRawProofError("La clave publica embebida no es Ed25519")
    return key
