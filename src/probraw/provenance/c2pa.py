...
RAW_LINK_ASSERTION_LABEL = "org.probatia.probraw.raw-link.v1"
LEGACY_RAW_LINK_ASSERTION_LABELS = {
    "org.probatia.nexoraw.raw-link.v1",
    "org.probatia.nexoraw.raw-link",
    "org.probatia.iccraw.raw-link.v1",
    "org.probatia.iccraw.raw-link",
}
RENDER_SETTINGS_SCHEMA = "org.probatia.probraw.render-settings.v1"
PROBRAW_RENDER_ACTION = "org.probatia.probraw.rendered"
...
def c2pa_config_from_environment(
    *,
    ...
    cert = _env_first("PROBRAW_C2PA_CERT", "NEXORAW_C2PA_CERT")
    key = _env_first("PROBRAW_C2PA_KEY", "NEXORAW_C2PA_KEY")
    if not cert or not key:
        raise C2PASigningError(
            "No hay credenciales C2PA configuradas. Configura PROBRAW_C2PA_CERT "
            "y PROBRAW_C2PA_KEY, o pasa --c2pa-cert y --c2pa-key en la CLI, "
            "solo si quieres incrustar la capa C2PA opcional."
        )

    env_manifest = _env_first("PROBRAW_C2PA_TECHNICAL_MANIFEST", "NEXORAW_C2PA_TECHNICAL_MANIFEST")
    env_session = _env_first("PROBRAW_SESSION_ID", "NEXORAW_SESSION_ID")
    return C2PASignConfig(
        cert_path=Path(cert).expanduser(),
        key_path=Path(key).expanduser(),
        alg=_env_first("PROBRAW_C2PA_ALG", "NEXORAW_C2PA_ALG") or "ps256",
        timestamp_url=_env_first("PROBRAW_C2PA_TIMESTAMP_URL", "NEXORAW_C2PA_TIMESTAMP_URL") or DEFAULT_TIMESTAMP_URL,
        signer_name=_env_first("PROBRAW_C2PA_SIGNER_NAME", "NEXORAW_C2PA_SIGNER_NAME") or "ProbRAW",
        ...
        key_passphrase=_env_first("PROBRAW_C2PA_KEY_PASSPHRASE", "NEXORAW_C2PA_KEY_PASSPHRASE"),
    )
...
def auto_c2pa_config(
    *,
    technical_manifest_path: Path | None = None,
    session_id: str | None = None,
    signer_name: str = "ProbRAW local signer",
    timestamp_url: str | None = DEFAULT_TIMESTAMP_URL,
) -> C2PASignConfig | None:
    try:
...
def ensure_local_c2pa_identity(
    *,
    ...
    private_key = base / "probraw-c2pa-private-key.pem"
    cert_chain = base / "probraw-c2pa-cert-chain.pem"
    root_cert = base / "probraw-c2pa-local-root.pem"
    ...
    root_name = x509.Name(
        [
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "ProbRAW local trust"),
            x509.NameAttribute(NameOID.COMMON_NAME, "ProbRAW local C2PA root"),
        ]
    )
    leaf_name = x509.Name(
        [
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "ProbRAW local signer"),
            x509.NameAttribute(NameOID.COMMON_NAME, signer_name[:64] or "ProbRAW local signer"),
        ]
    )
    root = (
...
def _default_c2pa_identity_dir() -> Path:
    base = _env_first("PROBRAW_HOME", "NEXORAW_HOME")
    if base:
        return Path(base).expanduser() / "c2pa"
    return Path.home().expanduser() / ".probraw" / "c2pa"
...
def build_c2pa_manifest(
    *,
    ...
    return {
        "claim_generator": f"ProbRAW/{__version__}",
                    ...
                    "actions": [
                        {
                            ...
                            "parameters": {
                                "description": "Rendered TIFF generated by ProbRAW from a camera RAW source.",
                                "raw_sha256": raw_sha,
                                "raw_link_assertion": RAW_LINK_ASSERTION_LABEL,
                            },
                        },
                        {
                            "action": PROBRAW_RENDER_ACTION,
                            "softwareAgent": software_agent,
                            "parameters": {
                                "raw_sha256": raw_sha,
                                "recipe_sha256": raw_link_assertion["probraw"]["recipe_sha256"],
                                "icc_profile_sha256": raw_link_assertion["probraw"]["icc_profile_sha256"],
                                "demosaicing_algorithm": raw_link_assertion["probraw"]["demosaicing_algorithm"],
                                "output_space": raw_link_assertion["probraw"]["output_space"],
                                "color_management_mode": raw_link_assertion["probraw"]["color_management_mode"],
                                "render_settings_sha256": raw_link_assertion["probraw"].get("render_settings_sha256"),
                            },
                        },
                    ]
                },
...
def sign_tiff_with_c2pa(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="probraw_c2pa_") as tmp:
...
def _verify_render_settings_hash(raw_link: dict[str, Any]) -> dict[str, Any]:
    ...
    declared = render_settings.get("settings_sha256") or _nested_get(raw_link, ("probraw", "render_settings_sha256"))
    if not declared:
