...
def inspect_file_metadata(
    ...
    *,
    ...
    payload = {
        ...
        "probraw_proof": read_probraw_proof_metadata(path),
    }
    if include_c2pa:
...
def read_probraw_proof_metadata(path: Path) -> dict[str, Any]:
    path = Path(path)
    proof_path = path if path.suffix.lower() == ".json" else existing_proof_sidecar_path(path)
    ...
    if raw_payload.get("schema") not in {PROOF_SCHEMA, *LEGACY_PROOF_SCHEMAS}:
        return {"status": "not_probraw_proof", "proof_path": str(proof_path)}
    try:
        result = verify_probraw_proof(
            proof_path,
            output_tiff=path if path != proof_path and path.exists() else None,
        )
    except ProbRawProofError as exc:
...
def _summary_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "probraw_proof_status": proof.get("status") if isinstance(proof, dict) else None,
        "probraw_proof_path": proof.get("proof_path") if isinstance(proof, dict) else None,
    }
...
def _interpreted_summary(
    ...
    proof = payload.get("probraw_proof", {}) if isinstance(payload.get("probraw_proof"), dict) else {}
    return [
        {
            "title": "Archivo",
            "items": _items(
        ...
        },
        {
            ...
            "items": _items(
                ("ProbRAW Proof", _proof_status_label(proof)),
...
def _proof_display(proof: dict[str, Any]) -> list[dict[str, Any]]:
    if not isinstance(proof, dict):
        return [{"title": "ProbRAW Proof", "items": [{"label": "Estado", "value": "No disponible"}]}]
    ...
    ] + _render_settings_display("Ajustes TIFF ProbRAW Proof", render_settings)
...
def _c2pa_display(c2pa: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    probraw = raw_link.get("probraw", {}) if isinstance(raw_link, dict) else {}
    ...
    return [
        ...
        },
        {
            ...
            "items": _items(
                ...
                ("Hash receta", probraw.get("recipe_sha256")),
                ("Hash ICC", probraw.get("icc_profile_sha256")),
                ("Hash ajustes render", probraw.get("render_settings_sha256")),
                ("Backend RAW", probraw.get("raw_backend")),
                ("Demosaicing", probraw.get("demosaicing_algorithm")),
                ("Espacio salida", probraw.get("output_space")),
                ("Modo color", probraw.get("color_management_mode")),
                ("Generado UTC", probraw.get("generated_at_utc")),
            ),
        },
        *_render_settings_display("Ajustes TIFF ProbRAW C2PA", render_settings),
        {
            "title": "Validacion",
...
def _proof_status_label(proof: dict[str, Any]) -> str | None:
    ...
    if status == "ok":
        return "ProbRAW Proof valido"
    labels = {
        "absent": "Sin sidecar ProbRAW Proof",
        "invalid": "ProbRAW Proof no valido",
        "error": "Error de lectura/verificacion Proof",
        "not_probraw_proof": "JSON no es ProbRAW Proof",
        "failed": "ProbRAW Proof fallido",
    }
    return labels.get(status, _format_value(status) if status else None)
...
def _c2pa_next_step(c2pa: dict[str, Any]) -> str:
    ...
    if status == "absent_or_invalid":
        return "Los TIFF finales de ProbRAW deben exportarse con C2PA; vuelve a renderizar con firma configurada."
    if status == "skipped":
        return "Activar la lectura C2PA para inspeccionar credenciales embebidas."
    return "Revisar el motivo mostrado y conservar el manifiesto externo de ProbRAW."
