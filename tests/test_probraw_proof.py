...
from probraw.core.utils import write_tiff16
from probraw.provenance.c2pa import build_render_settings
from probraw.provenance.probraw_proof import (
    ProbRawProofConfig,
...
def test_probraw_proof_signs_and_verifies_raw_tiff_link(tmp_path: Path):
...
def test_probraw_proof_detects_modified_tiff(tmp_path: Path):
    raw = tmp_path / "capture.NEF"
    tiff = tmp_path / "capture.tiff"
    private_key = tmp_path / "proof-private.pem"
    public_key = tmp_path / "proof-public.pem"
    ...
    result = sign_probraw_proof(
        ...
        config=ProbRawProofConfig(private_key_path=private_key, public_key_path=public_key),
    )
    tiff.write_bytes(tiff.read_bytes() + b"tamper")

    verified = verify_probraw_proof(Path(result.proof_path), output_tiff=tiff, source_raw=raw)
