...
ProbRAW 0.3.7 is suitable for controlled testing, method review and release
candidate validation. It is not yet a certified scientific or forensic
production system.

The current version tightens analysis and color behavior: preview matches the
exported TIFF in color-managed applications, real-scale zoom does not
interpolate pixels, MTF/Auto Sharpness use richer metrics and MTF work is paused
outside the Sharpness tab so chromatic adjustments stay responsive.
