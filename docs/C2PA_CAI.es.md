...
ProbRAW incorpora C2PA como una capa interoperable sobre los TIFF finales,
...
`ProbRAW Proof`: `batch_manifest.json`, hashes SHA-256, auditoria lineal,
perfiles ICC, reportes QA y sidecars siguen siendo parte del flujo principal.

La firma obligatoria del flujo ProbRAW es `ProbRAW Proof`, documentada en
[ProbRAW Proof](PROBRAW_PROOF.md). C2PA se incrusta con certificado externo si
existe; si no, ProbRAW crea una identidad local autoemitida y la usa como firma
C2PA de laboratorio.

## Principios de implementacion

1. El RAW original no se modifica nunca.
2. ProbRAW no incrusta C2PA en RAW propietarios originales (`CR2`, `CR3`,
   `NEF`, `ARW`, `RAF`, `RW2`, `ORF`, `PEF`).
3. Para RAW propietarios, el vinculo probatorio se declara en el TIFF final con
   una asercion C2PA firmada: `org.probatia.probraw.raw-link.v1`.
...
5. El SHA-256 del TIFF firmado se calcula despues de incrustar C2PA y se guarda
   en el manifiesto externo de ProbRAW. No se incluye dentro del manifiesto C2PA
   embebido para evitar una referencia circular.
6. Si el origen es DNG, ProbRAW intenta pasarlo al SDK C2PA como ingrediente
...
9. ProbRAW no depende de una lista central de confianza C2PA para la prueba
   autonoma. Si falta el SDK C2PA, el TIFF se exporta con sidecar ProbRAW Proof.
...
Para no depender de certificados emitidos por una autoridad CAI, ProbRAW genera
...
modelo de ProbRAW esto es una advertencia de confianza externa: la firma no esta
...
`ProbRAW Proof` y `batch_manifest.json`.
...
Si no se pasan `--c2pa-cert` y `--c2pa-key`, ProbRAW intenta usar primero
...
privada PEM. Si no se indica otra TSA, ProbRAW usa
...
nombre final y se firma el sidecar ProbRAW Proof con el hash exacto de ese TIFF.
`batch_manifest.json` guarda el hash del TIFF final y el hash del proof.

## Asercion RAW -> TIFF

La asercion `org.probatia.probraw.raw-link.v1` registra:
...
- Resumen reproducible de ajustes dentro del bloque `probraw`, duplicando los
...
ProbRAW 0.2.5 y versiones posteriores generan etiquetas
`org.probatia.probraw.*`. El verificador sigue aceptando etiquetas heredadas
...
Esta lectura combina EXIF/GPS mediante `exiftool`, ProbRAW Proof mediante el
sidecar `.probraw.proof.json` y manifiestos C2PA mediante `c2pa-python` si el
