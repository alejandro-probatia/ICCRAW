...
gh repo edit alejandro-probatia/NexoRAW \
  ...
  --add-topic forensic-photography \
  --add-topic colorimetry \
  --add-topic argyllcms \
  --add-topic libraw \
  --add-topic colorchecker \
  --add-topic digital-forensics \
  --add-topic reproducible-research \
  --add-topic scientific-imaging \
  --add-topic cultural-heritage \
  --add-topic conservation \
  --add-topic c2pa \
  --add-topic agpl
```
